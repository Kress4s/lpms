...
func (igi *ImplementGovRepoImpl) List(db *gorm.DB, pageInfo *vo.PageInfo, params *vo.ImplementGovFilterParam) (int64,
	...
	tx := db.Table(tables.ImplementGov).Select("id, name, level, project_type, construct_subject, create_at, status, start_time, finish_time").Where("status <> ? and status <> ?", constant.StartInspecting, constant.FinishInspect)
	if params.Name != "" {
