...
type ImpleIndustry struct {
	...
	PointType               *int            `gorm:"column:point_type;type:integer;comment:重点类型 0:省重点实施项目,1:省重点预备项目,2:省重大产业项目;3:省4+1项目;4:省6千亿项目;5:市重点实施项目;6:市重点预备项目;7:无重点类型;8: 152工程"`
...
func (ImpleIndustry) TableName() string {
	return tables.ImplementIndustry
}

func (b *ImpleIndustry) BeforeCreate(tx *gorm.DB) error {
	now := time.Now()
	b.CreateAt = now
	b.UpdateAt = now
	return nil
...
func (b *ImpleIndustry) BeforeUpdate(tx *gorm.DB) error {
	b.UpdateAt = time.Now()
	return nil
}
